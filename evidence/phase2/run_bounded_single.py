#!/usr/bin/env python3
import datetime,hashlib,json,re,statistics,subprocess,sys,time,csv
...
src=root/'evidence/phase2/binary_search_bounded_single.cu';binary=out/'build/single'
...
try:
    ...
    assert 'PASS queries=150994944 timed=0' in (out/'check.stdout.log').read_text()
    run('sass',['/usr/local/cuda/bin/cuobjdump','--dump-sass',str(binary)],30)
    ...
    content=(out/'measure.stdout.log').read_text();assert 'PASS queries=905969664 timed=1' in content
    groups={}
    for n,s,r,v,i,ms in re.findall(r'SAMPLE n=(\d+) shape=(\d+) round=(\d+) variant=(\d+) sample=(\d+) ms=([\d.]+)',content):groups.setdefault(tuple(map(int,(n,s,r,v))),[]).append(float(ms))
    assert len(groups)==108 and all(len(x)==30 for x in groups.values())
    ...
    for part in (out/'sass.stdout.log').read_text().split('Function : ')[1:]:
        match=re.match(r'_Z14bounded_kernelILi([012])ELb0EE',part)
        if match:instructions[int(match[1])]='\n'.join(re.sub(r'/\*.*?\*/','',line).strip() for line in part.splitlines() if re.match(r'\s*/\*[0-9a-f]+\*/.*;',line))
    m['single_official_clone_instruction_identity']=instructions[0]==instructions[1]
    m['single_sass']={v:dict(lines=len(t.splitlines()),sha256=hashlib.sha256(t.encode()).hexdigest()) for v,t in instructions.items()}
